...
export const OPTIONS = {
  ...
  format: { alias: 'f', kind: 'value', describe: 'md | json | html | csv | ndjson' },
  all: { kind: 'boolean', describe: 'Every report in the folder, not only one' },
  column: { kind: 'value', describe: 'impact: a physical or model column' },
  measure: { kind: 'value', describe: 'impact: a measure' },
  'fail-if-used': { kind: 'boolean', describe: 'impact: exit 1 when anything reads the target' },
  'min-coverage': { kind: 'value', describe: 'Fail below this % of columns resolved to a physical source' },
  'fail-on': { kind: 'value', describe: 'Comma-separated check rules to fail the build on' },
  baseline: { kind: 'value', describe: 'check: fail only on findings not in this file' },
  'write-baseline': { kind: 'value', describe: 'check: record current findings as known debt' },
  'update-baseline': { kind: 'boolean', describe: 'check: drop fixed findings from --baseline' },
  'page-maps': { kind: 'boolean', describe: 'docs: a small page map per visual' },
