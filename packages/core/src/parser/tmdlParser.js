...
export function parseTableFile(content, fileName) {
  ...
  while (i < lines.length) {
    ...
    if (indent === 1 || (indent === 0 && !trimmed.startsWith('table '))) {
      // Column definition. The name is either quoted or a bare token, and may be
      // followed by `= <DAX>` for a calculated column — the same inline form measures
      // use. Splitting on the name first matters: taking everything after `column ` as
      // the name turned `column 'Is Past Date' = 'Time Period'[Date] <= TODAY()` into a
      // column literally named that whole line, so the real column went missing, its DAX
      // was never parsed, and every measure reading it looked like a broken reference.
      const colMatch = trimmed.match(/^column\s+(?:'([^']*)'|([^\s=]+))\s*(?:=\s*(.*))?$/);
      if (colMatch) {
        const name = colMatch[1] ?? unquoteName(colMatch[2].trim());
        const inlineExpr = colMatch[3];
        const col = parseColumnBlock(lines, i, name);

        if (inlineExpr !== undefined) {
          // Multi-line DAX continues on the indented lines below, exactly as for a
          // measure, so the shared extractor handles both.
          col.expression = extractDaxExpression(lines, i) || inlineExpr.trim();
        }
...
/**
 * TMDL boolean properties, which are written as a bare word with no colon.
 * Anything here terminates an expression rather than continuing it.
 */
const BARE_FLAGS = new Set([
  'isHidden', 'isPrivate', 'isNullable', 'isUnique', 'isKey', 'isDefault',
  'isDefaultImage', 'isDefaultLabel', 'isAvailableInMdx', 'isDataTypeInferred',
  'isNameInferred', 'showAsVariationsOnly', 'isParameterQuery',
]);
...
export function extractDaxExpression(lines, startIndex) {
  ...
  while (i < lines.length) {
    ...
    // Bare boolean flags carry no colon, so the rule above walks straight past them and
    // appends `isHidden` to the DAX. Matching a lone word generally would be wrong —
    // `RETURN` on one line and `_result` on the next is ordinary DAX — so this is a
    // list of the flags TMDL actually defines.
    if (BARE_FLAGS.has(lineTrimmed)) break;

    // A TMDL sub-block (`annotation X = …`, `changedProperty = …`) ends the expression
    // for the same reason, and would otherwise be read as more DAX.
    if (/^(annotation|changedProperty|extendedProperty|relatedColumnDetails)\b/.test(lineTrimmed)) break;
...
function parsePartitionBlock(lines, startIndex, name, type) {
  ...
  while (i < lines.length) {
    ...
    // mode: import / directQuery / directLake
    ...
    // A Direct Lake partition is `= entity`, and its `source` block is properties rather
    // than an M expression:
    //
    ...
    //
    // These three lines say outright what an M chain has to be walked to discover, so a
    // Direct Lake table is the *easiest* kind to resolve — but only once they are kept.
    // They were being dropped, which left the main fact table of a Direct Lake model with
    // no physical source at all.
    const entityMatch = trimmed.match(/^(entityName|schemaName|expressionSource)\s*[:=]\s*(.+)$/i);
    if (entityMatch) {
      const key = entityMatch[1].charAt(0).toLowerCase() + entityMatch[1].slice(1);
      partition[key] = unquoteName(entityMatch[2].trim());
      i++;
      continue;
    }
...
export function parseRelationships(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (fromColMatch) {
      ...
      const dotMatch = splitColumnRef(ref);
      if (bracketMatch) {
        current.fromTable = unquoteName(bracketMatch[1].trim());
        current.fromColumn = unquoteName(bracketMatch[2].trim());
      } else if (dotMatch) {
        current.fromTable = dotMatch.table;
        current.fromColumn = dotMatch.column;
      } else {
    ...
    if (toColMatch) {
      ...
      if (bracketMatch) {
        ...
        current.toTable = dotMatch.table;
        current.toColumn = dotMatch.column;
      } else {
...
/**
 * Split a TMDL relationship endpoint into its table and column halves.
 *
 * Either half is single-quoted when its name is not a bare identifier, so all four
 * combinations occur — three of them in one real model:
 *
 *   'Time Period'.date_sk     quoted table, bare column     (70 of 87)
 *   Range.'Category Name'          bare table, quoted column      (1)
 *   Sales.Amount              neither                       (16)
 *   'A B'.'C D'               both
 *
 * The rule is the *first dot outside quotes*: a bare table name cannot contain one, and
 * a quoted name's dots are protected. The previous pattern required the column half to
 * be `\w+`, so `Range.'Category Name'` matched nothing and fell through to a fallback that
 * put the whole reference in the column and left the table empty. That relationship then
 * read as one pointing at a table the model does not contain — a parser artefact wearing
 * the costume of a model defect, found by a check built to report exactly that.
 *
 * @param {string} ref
...
function splitColumnRef(ref) {
  ...
  for (let i = 0; i < ref.length; i++) {
    ...
    if (ch === "'") quoted = !quoted;
      ...
      const table = unquoteName(ref.slice(0, i).trim());
      const column = unquoteName(ref.slice(i + 1).trim());
