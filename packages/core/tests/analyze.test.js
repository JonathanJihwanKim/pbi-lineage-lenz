...
describe('extractMDataSource', () => {
  it('handles concatenated SQL in Value.NativeQuery', () => {
    const mExpr = `let
    Source = Value.NativeQuery(GoogleBigQuery.Database([BillingProject=_BillingProject]), "SELECT * FROM \`" & _BillingProject & ".report_stores.store_cur_func_dim\` ('" & _ReportId & "')", null, [EnableFolding=true])
in
    Source`;
    const ds = extractMDataSource(mExpr);
    expect(ds.sourceTable).toBe('store_cur_func_dim');
    expect(ds.database).toBe('report_stores');
  });
});
