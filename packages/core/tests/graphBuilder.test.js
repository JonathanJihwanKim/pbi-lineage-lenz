...
describe('buildGraph – DAX unqualified measure extraction', () => {
  ...
  it('captures unqualified measure refs that appear after quoted table names', () => {
    ...
    // [Number of Orders with Exception] appears AFTER 'Store'[...]
    const tables = [
      {
        name: 'Measure',
        columns: [],
          ...
          { name: 'Number of Orders with Exception', expression: 'SUM(Fact[exception_count])' },
          {
            ...
            expression: `DIVIDE(
              CALCULATE( [Number of Completed Orders],
                FILTER ( 'Store', 'Store'[Store Type] = "STO" )
      ...
      {
        name: 'Store',
        columns: [{ name: 'Store Type', dataType: 'string' }],
        measures: []
      },
      {
        name: 'Fact',
    ...
    const tables = [
      {
        ...
        measures: [
          ...
          { name: 'Customer Order Quality Deviation', expression: 'SUM(Fact[dev])' },
          {
            ...
            expression: "CALCULATE( DIVIDE( [Number of Completed Orders] - [Customer Order Quality Deviation], [Number of Completed Orders] ), KEEPFILTERS( 'Store'[Store Type] = \"STO\" ) )"
          }
        ]
      },
      {
